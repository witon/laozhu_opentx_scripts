...
- **`SHOW_LOG_SCREEN`**：为真且调用对应级别的函数未被开关短路时，该行写入环形缓冲 **`DBG.logHistory`**，由 [`DBGLogListView.lua`](LAOZHU/DBGTools/DBGLogListView.lua) 的 **`DBGLogLVdraw`**（及配套的 **`DBGLogLVmaxVisForRect`** 用于 `DBG_logClampScroll`）绘制。
...
4. 若需在 Widget / 遥测上绘制日志：`LZ_runModule(... "LAOZHU/DBGTools/DBGLogListView.lua")`，按矩形调用 `DBGLogLVmaxVisForRect` → `DBG_logClampScroll` → `DBGLogLVdraw`（布局使用 `LZ_ui`，见 UiParams）。
...
- **黑白屏遥测**：与 Widget 共用 [`DBGLogListView.lua`](LAOZHU/DBGTools/DBGLogListView.lua)；写入规则一致（读 `DBG.logHistory`，行内混排 `[ERR]`/`[DBG]`）。例如 `utO` 遥测在 `utOFramework` 中于屏幕底部带状区域调用 `DBGLogLVdraw`。
