...
function RLVdraw(recordListView, x, y, invers, option)
    ...
    if records ~= nil then
        ...
        for i=scrollRow+1, #records, 1 do
            ...
            local y = 30 + (i-scrollRow-1) * rs
            local op = 0
            if i==recordListView.selectedRow and recordListView.focusState == 2 then
                op = INVERS
                lcd.drawFilledRectangle(0, y-1, 127, rs, FORCE)
            end
            lcd.drawText(0, y, LZ_formatTimeStamp(record.startTime), LZ_ui.font + LEFT + op)
            lcd.drawText(57, y, record.ele, LZ_ui.font + RIGHT + op)
            lcd.drawText(80, y, record.flap1, LZ_ui.font + RIGHT + op)
            lcd.drawText(103, y, record.flap2, LZ_ui.font + RIGHT + op)
            lcd.drawNumber(128, y, SRRgetRecordSinkRate(record), LZ_ui.font + RIGHT + op)
            if record.invalid then
                local ym = y + math.floor(rs / 2)
                if op == INVERS then
