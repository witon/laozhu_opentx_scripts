...
function testSinkRateStateNormalProcess()
    ...
    SRSrun(sinkRateState, 300, 30, 1000)
    local curDur = SRSgetCurDuration(sinkRateState)
    luaunit.assertEquals(curDur, 100)
    local curSinkAlt = SRSgetCurSinkAlt(sinkRateState)
    luaunit.assertEquals(curSinkAlt, 10)
    local curSinkRate = SRSgetCurSinkRate(sinkRateState)
